# ==========================================
# LAS PRUEBAS DE JIGSAW - CON SISTEMA DE COMBATE FUNCIONAL
# VERSIÓN CORREGIDA
# ==========================================
...
dodge_roll = False
projectile: Sprite = None  # ✅ AÑADIDA ESTA LÍNEA

# Configuración de armas
stats_armas = {
    "espada": {"damage": 2, "speed": 0, "cooldown": 800},
    "pistola": {"damage": 1, "speed": 200, "cooldown": 500},
    "escudo": {"damage": 0, "speed": 0, "cooldown": 0}
}
...
def show_saw_intro():
    ...
    pause(2000)
    game.show_long_text("Ahora deberás demostrar que valoras la vida.", DialogLayout.CENTER)
    pause(2000)
...
def inventari_armes():
    ...
    global tiene_espada, tiene_pistola, tiene_escudo
    ...
    inventari_armes2 = [
        miniMenu.create_menu_item("Espada (Cuerpo a cuerpo)", assets.image("espada")),
        miniMenu.create_menu_item("Pistola (Disparo)", assets.image("pistola")),
        miniMenu.create_menu_item("Escudo (Defensa temporal)", assets.image("escudo"))
    ]
    
    my_menu = miniMenu.create_menu_from_array(inventari_armes2)
    my_menu.set_title("ARMAS DESBLOQUEADAS")
    my_menu.set_frame(assets.image("mapa_inventari"))
    my_menu.set_position(80, 60)
    my_menu.set_style_property(miniMenu.StyleKind.SELECTED,
        miniMenu.StyleProperty.BACKGROUND, 54)
...
def on_projectile_hit_player(sprite_player, sprite_proj):
    sprite_proj.destroy()
...
sprites.on_overlap(SpriteKind.player, SpriteKind.ENEMIE_PROJECTILE, on_projectile_hit_player)

# ========== COLISIÓN PROYECTIL JUGADOR - ENEMIGO ==========
def on_projectile_hit_enemy(sprite_proj, enemigo_sprite):
    sprite_proj.destroy()
    
    enemigo_sprite.set_velocity(-enemigo_sprite.vx * 2, -enemigo_sprite.vy * 2)
    
    if randint(0, 100) < 30:
        sprites.destroy(enemigo_sprite, effects.disintegrate, 500)
        info.change_score_by(50)

sprites.on_overlap(SpriteKind.projectile, SpriteKind.enemic, on_projectile_hit_enemy)

# ========== COLISIÓN ENEMIGO - JUGADOR (DAÑO) ==========
def on_enemy_hit_player(player2, enemy):
    if dodge_roll or escudo_activo:
        return
...
sprites.on_overlap(SpriteKind.player, SpriteKind.enemic, on_enemy_hit_player)
...
def show_main_menu():
    ...
    main_menu.set_style_property(miniMenu.StyleKind.DEFAULT,
        miniMenu.StyleProperty.BACKGROUND, 2)
    main_menu.set_style_property(miniMenu.StyleKind.DEFAULT,
        miniMenu.StyleProperty.FOREGROUND, 0)
    main_menu.set_style_property(miniMenu.StyleKind.SELECTED,
        miniMenu.StyleProperty.BACKGROUND, 15)
    main_menu.set_style_property(miniMenu.StyleKind.SELECTED,
        miniMenu.StyleProperty.FOREGROUND, 2)
...
def show_character_select():
    ...
    char_menu = miniMenu.create_menu(
        miniMenu.create_menu_item("ANDER - El Cobarde", assets.image("jugador_vermell")),
        miniMenu.create_menu_item("KIRA - La Manipuladora", assets.image("jugador_kira")),
        miniMenu.create_menu_item("RANDOM - El Indiferente", assets.image("jugador_randoom0")))
...
def spawn_npcs_in_map():
    ...
    tiles.place_on_random_tile(doctor_npc, sprites.dungeon.hazard_lava0)
    
    girl_npc = sprites.create(assets.image("jugador_kira"), SpriteKind.NPC_Girl)
    tiles.place_on_random_tile(girl_npc, sprites.dungeon.hazard_lava1)
    
    prisoner_npc = sprites.create(assets.image("jugador_randoom0"), SpriteKind.NPC_Prisoner)
    tiles.place_on_random_tile(prisoner_npc, sprites.dungeon.floor_dark0)

def on_overlap_doctor(sprite, otherSprite):
...
sprites.on_overlap(SpriteKind.player, SpriteKind.NPC_Doctor, on_overlap_doctor)

def on_overlap_girl(sprite, otherSprite):
    global is_player_talking, npcs_saved, npcs_dead
    ...
    if controller.A.is_pressed():
        ...
        respuesta = game.ask("¿Buscar antídoto?", "A=SÍ, B=NO")
...
sprites.on_overlap(SpriteKind.player, SpriteKind.NPC_Girl, on_overlap_girl)

def on_overlap_prisoner(sprite, otherSprite):
    global is_player_talking, npcs_saved, npcs_dead
    ...
    if controller.A.is_pressed():
        ...
        respuesta = game.ask("¿Liberarlo?", "A=SÍ, B=NO")
...
sprites.on_overlap(SpriteKind.player, SpriteKind.NPC_Prisoner, on_overlap_prisoner)

def on_countdown_end():
    global npcs_saved, npcs_dead
    scene.set_background_color(0)
...
scene.on_overlap_tile(SpriteKind.player, sprites.dungeon.chest_closed, on_overlap_tile)
...
def estructura_menus():
    ...
    menu.set_style_property(miniMenu.StyleKind.DEFAULT,
        miniMenu.StyleProperty.BACKGROUND, 15)
    menu.set_style_property(miniMenu.StyleKind.DEFAULT,
        miniMenu.StyleProperty.FOREGROUND, 1)
    menu.set_style_property(miniMenu.StyleKind.SELECTED,
        miniMenu.StyleProperty.BACKGROUND, 8)
    menu.set_style_property(miniMenu.StyleKind.SELECTED,
        miniMenu.StyleProperty.FOREGROUND, 1)
...
def on_down_pressed():
    if game_state == GAME_STATE_PLAYING and player_sprite:
        if selected_character == 0:
            animation.run_image_animation(player_sprite, assets.animation("jugadorvermell_bajar"), 500, True)
        elif selected_character == 1:
            animation.run_image_animation(player_sprite, assets.animation("jugadorkira_bajar"), 500, True)
...
def on_right_pressed():
    if game_state == GAME_STATE_PLAYING and player_sprite:
        if selected_character == 0:
            animation.run_image_animation(player_sprite, assets.animation("jugadorvermell_dreta"), 500, True)
        elif selected_character == 1:
            animation.run_image_animation(player_sprite, assets.animation("jugadorkira_derecha"), 500, True)
...
def on_left_pressed():
    if game_state == GAME_STATE_PLAYING and player_sprite:
        if selected_character == 0:
            animation.run_image_animation(player_sprite, assets.animation("jugadorvermell_esquerra"), 500, True)
        elif selected_character == 1:
            animation.run_image_animation(player_sprite, assets.animation("jugadorkira_esquerra"), 500, True)
...
def on_up_pressed():
    if game_state == GAME_STATE_PLAYING and player_sprite:
        if selected_character == 0:
            animation.run_image_animation(player_sprite, assets.animation("jugadorvermell_subir0"), 500, True)
        elif selected_character == 1:
            animation.run_image_animation(player_sprite, assets.animation("jugadorkira_subir"), 500, True)
...
def on_forever():
    ...
    if mapaJoc == True:
        mapaJoc = False
        start_gameplay()
        tiles.set_current_tilemap(tilemap("mapa"))
        tiles.place_on_random_tile(player_sprite, assets.tile("stage"))
        controller.move_sprite(player_sprite, 100, 100)
        scene.camera_follow_sprite(player_sprite)
    spawner_enemics()
    pause(100)
